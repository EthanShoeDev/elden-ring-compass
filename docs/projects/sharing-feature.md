...
> The plumbing for a _lossy curated_ share is already on `main` — `lib/share/` (encode/decode,
...
> **missing**: (a) any UI that actually _builds_ a link (only tests call `encodeToUrl`), (b) a
...
- **The unit is one `LeanSlot`** (a character), not the whole `LeanSave` (5 slots). Sharing one
  character is both smaller _and_ the right UX.
- **A full-fidelity single slot ≈ 11 KB** in the query string (brotli + base64url). A
  **render-only single slot ≈ 7 KB.** Both fit a URL.
- **The binding constraint is not the browser — it's the _server_.** When the recipient opens
...
| event-flag bitfield _alone_ (1.71 MB, 99.9% zeros) |              — |            — |           — |         **3,868** |
...
1. **The event-flag bitfield is huge but nearly empty** — 1.71 MB, 99.9% zeros, so it brotli's
   down to ~3.9 KB on its own. Keeping _all_ flags (for a future complete-fidelity quest share)
...
3. **Brotli over _raw binary_ wins decisively.** Compressing the binary blob then base64'ing
   (≈2× better than LZString, and better than brotli-ing a base64 _string_). The current v1 uses
...
> _is_ the extracted subset). The remaining trim levers live _inside_ `LeanSlot`: the full
...
There are two regimes, and the share link hits _both_ (it's generated client-side but opened cold
by the recipient):
...
| Browser address bar _display_    |                                          Chrome truncates ~32 KB shown | Cosmetic; navigation still works above this.              |
...
is _never sent to the server_, so only browser limits (~MBs) apply. Cost: it's invisible to
...
- **Codec:** brotli over a binary blob → base64url. Browser brotli needs a wasm dep
  ...
  - native `CompressionStream('gzip')`: render-only ≈ **9.8 KB** (zero deps, but _over_ 8 KB nginx).
...
- Validate the _decoded_ payload with the existing effect `Schema` (a `LeanSlot` subset Struct),
...
The search schema validates only the opaque `{ save?: string }`; the _rich_ validation happens in
...
1. **Server limit vs codec.** If we must guarantee <8 KB behind _any_ proxy, we need `brotli-wasm`
