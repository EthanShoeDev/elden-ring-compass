...
export type DefType =
  | 's8' | 'u8' | 's16' | 'u16' | 's32' | 'u32' | 'b32'
  | 'f32' | 'angle32' | 'f64' | 'fixstr' | 'fixstrW' | 'dummy8';
...
export const parseParamdefXml = (xml: string): Effect.Effect<Paramdef, ParamdefError> =>
  Effect.gen(function* () {
    const paramType = xml.match(/<ParamType>([^<]+)<\/ParamType>/)?.[1]?.trim();
    if (!paramType) {
      return yield* new ParamdefError({ detail: 'no <ParamType> in def XML' });
    }
    const dataVersion = Number(xml.match(/<DataVersion>(\d+)<\/DataVersion>/)?.[1] ?? -1);
    ...
    for (const m of xml.matchAll(/<Field\s+Def="([^"]+)"/g)) {
      ...
      if (!parsed) return yield* new ParamdefError({ detail: `unparseable Def "${def}"` });
      const [, type, name, arr, bits] = parsed;
      if (!DEF_TYPES.has(type!)) {
        return yield* new ParamdefError({ detail: `unknown field type "${type}" in "${def}"` });
      }
      fields.push({
...
export const loadParamdef = (paramType: string): Effect.Effect<Paramdef, ParamdefError> =>
  Effect.gen(function* () {
    const url = new URL(`../vendor/paramdex/ER/Defs/${paramType}.xml`, import.meta.url);
    const xml = yield* Effect.tryPromise({
      try: () => Bun.file(url).text(),
      catch: (cause) => new ParamdefError({ detail: `reading def ${paramType}: ${String(cause)}` }),
    });
