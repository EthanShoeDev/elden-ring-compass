...
export const isDcx = (b: Uint8Array) =>
  b.length >= 4 && ascii(b, 0, 4) === 'DCX\0';
  ...
  Effect.gen(function* () {
    if (!isDcx(bytes)) {
      return yield* new DcxError({
        detail: `not a DCX (magic "${ascii(bytes, 0, 4)}")`,
      });
    }
    if (bytes.length < 0x18) {
      return yield* new DcxError({
        detail: `DCX too short (${bytes.length} bytes)`,
      });
    }
    ...
    if (
      dcsOffset + 12 > bytes.length ||
      dcpOffset + 8 > bytes.length ||
    ...
    return {
      format,
      uncompressedSize,
      compressed: bytes.subarray(dataOffset, end),
    };
  });
