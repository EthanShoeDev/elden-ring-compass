...
| **`er-common.emedf.json`** (388 instrs, 415 KB) | **EMEVD instruction dictionary** — maps every event-script instruction `(bank, id)` to a name + typed arg layout. Drives `formats/emedf.ts` (`decodeInstruction`), the semantic layer for event-script extraction (#10b map treasure, quest compass). | **soulstruct** `src/soulstruct/eldenring/events/emevd/er-common.emedf.json` | A patch **adds/changes instructions** (rare — instruction set is very stable). Verified 100% opcode coverage on 107k instructions across 589 EMEVDs. | Unknown opcode (instruction not decoded) or wrong arg names/values.                                    | Re-copy from the soulstruct clone.                                                                                                                                                                          |
...
- **FMG text, PARAM rows, TPF textures, MSB/EMEVD** — all read live from the unpacked
  ...
  `bun run extract` against a patched install. (Their _parsers_ are our own code in
  `src/formats/`; the _content_ is the install's.)
