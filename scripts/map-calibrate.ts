...
for (let coff = -40; coff <= 40; coff++)
  for (let roff = -40; roff <= 40; roff++) {
    ...
    for (const g of graces)
      if (menu.has(`${g.col + coff},${g.row + roff}`)) hits++;
    if (hits > best.hits) best = { coff, roff, hits };
  }
const COFF = best.coff,
  ROFF = best.roff;
const worldToPixel = (X: number, Z: number): [number, number] => [
  X + 256 * COFF,
  (41 - ROFF) * 256 - Z,
];
console.log(
  `[align] offset COFF=${COFF} ROFF=${ROFF} -> ${best.hits}/${graces.length} graces on existing tiles`,
);
console.log(
  `[align] masterPx = worldX + ${256 * COFF}   masterPy = ${(41 - ROFF) * 256} - worldZ`,
);
const inB = graces.filter((g) => {
  const [px, py] = worldToPixel(g.X, g.Z);
  return px >= 0 && px <= 10496 && py >= 0 && py <= 10496;
}).length;
...
console.log(
  `[world→wiki] isotropy: 1/slopeY=${(1 / wy.slope).toFixed(2)}px  1/slopeX=${(1 / wx.slope).toFixed(2)}px (equal ⇒ square pixels, confirms 1:1)`,
);
...
console.log(
  `[wiki→pixel] py = ${py.slope.toFixed(4)}*wikiX + ${py.intercept.toFixed(2)}   (r2=${py.r2.toFixed(5)}, mean|res|=${py.mean.toFixed(1)}px, max=${py.max.toFixed(1)}px)`,
);
